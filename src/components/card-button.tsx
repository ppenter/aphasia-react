...
export const CardButton = ({title='iwant', to, img='/images/menu/wave.png', children, titleClass,indexColor=0, className}: any) => {
  const {t, i18n} = useTranslation(['default']);
  const color = genColor((indexColor + 1) * 4444)
    return(
      <Link to={to}>
      <div className={
        cn("flex flex-col items-center items-center gap-4 px-4 py-12 bg-white justify-evenly",
          className
        )
      }
      style={{
