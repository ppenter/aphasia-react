import React, { PropsWithChildren, useEffect, useState, useRef, useCallback, useMemo } from "react";
...
const CarouselListView: React.FC<ICarouselListView> = ({
  ...
  const toggleFavorite = useCallback((speech_id: string, pageItem: any) => {
  ...
  const CarouselGroup = React.memo(({ group, items }: { group: string; items: any[] }) => {
    ...
    const carouselItems = useMemo(() => {
      return items.reduce((acc: JSX.Element[], item, index) => {
        if (index % itemsPerPage === 0) {
          acc.push(
            <div key={index} className={`grid flex-shrink-0 w-full h-full gap-4`} style={{
              gridTemplateColumns: `repeat(${gridCols}, minmax(0, 1fr))`
            }}>
              {items.slice(index, index + itemsPerPage).map((pageItem) => {
                if (!pageItem?.title) return null;
                const speech_id = pageItem?.speech || `${title}.${pageItem?.title}.speech`;
                const img_url =
                  pageItem?.img ||
  ...
  const CarouselItem = React.memo(({ pageItem, speech_id, img_url, speak, toggleFavorite, t }: any) => {
