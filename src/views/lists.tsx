...
export const ListView: React.FC<IListView> = ({children, list=mockList, title}) => {
    ...
    return(
        <div className="flex flex-col p-4 pt-6">
            <div className="">
                {
                    group_list && Object.keys(group_list)?.map?.((group, index) => {
                        return (
                            <div key={group} className="grid col-span-1 gap-2">
                                {group !== "undefined" && <h3 className="text-4xl font-bold tracking-tight text-gray-900 dark:text-white">{t(`${title}.__group__.${group}`)}</h3>}
                                <div className="grid grid-cols-2 gap-4 pb-32 sm:grid-cols-3 lg:grid-cols-5">
                                    {
                                        group_list[group].map((item: any, index: number) => {
                                            if(!item?.title) return null
                                            const speech_id = item?.speech || `${title}.${item?.title}.speech`
                                            const img_url = item?.img || `/images/items/${title}/${item?.title?.toLocaleLowerCase()?.replaceAll(' ', '_')}.png`
                                                ...
                                                <CardButtonSmall title={item.title} img={
                                                    ...
                                                    <div className="flex flex-col items-center justify-center w-full max-w-full gap-4">
                                                        <h5 className={cn(
                                                            ...
                                                            {item?.custom ? item?.title : t(`${item?.id}.text`)}
                                                        </h5>
                                                        <img alt="heart" src={isFav ? HeartFull : Heart} className="absolute w-6 h-6 top-4 right-4" onClick={e => {
                                                            e.stopPropagation()
